...
- First we want to use the `count` function to make a long table with the number of people with each combination of `gender` and `call`.  (If you get confused, it might help to select all the code up to the end of the `count()` function and press "Run Code"!)
- Next we want to use the `pivot_wider` function to reshape the table. To do so, let's draw the names for the new columns from variable `call` and values from `n`.
...
In this exercise, we'll work with the ages of a sample of 11 people, stored in the `ages` vector. We can use relational operators to create a logical vector (that is, a vector whose values are `TRUE` or `FALSE`).  In this case, those values will indicate which ages fall within a specific range: in particular, we'll show whether each respondent is college-aged (18-22) or not.
