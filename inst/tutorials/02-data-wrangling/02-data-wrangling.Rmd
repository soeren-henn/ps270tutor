...
We often would like to get numerical summaries of the variables in our dataset. These might be means, medians, measures of spread, or other ways to summarize a collection of values. The `summarize()` function in `dplyr` consists of a number of name-value pairs where the value is some code that summarizes a variable and the name is the label we want to give that summary in the resulting data frame. Schematically, it looks like this:
...
Sometimes we need to create new variables that are transformations or mutations of existing variables. This is where the `mutate()` function comes in. This allows us to create new variables based on existing ones with the following syntax:
