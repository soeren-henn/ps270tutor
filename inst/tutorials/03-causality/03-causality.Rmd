...
We can use relational operators to create logical vectors.  Relational operators are operators such as `<`, `==`, or `<=` that compare two elements; several of these comparisons can be combined using logical operators like `&` or `|`.  In our `ages` sample, we can use comparisons to indicate whether an age falls within a specific range: in particular, we'll show whether each respondent is college-aged (18-22) or not.
